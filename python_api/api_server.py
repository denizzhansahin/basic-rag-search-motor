...
# YENİ ENDPOINT (WebSocket içindeki request hatası için)
@app.websocket("/ws/follow-up")
async def follow_up_endpoint(websocket: WebSocket):

    # --- EKLENEN GÜVENLİK BLOĞU ---
    client_ip = websocket.client.host
    token = websocket.query_params.get("token")

    if client_ip not in ACCEPT_URL:
        await websocket.close()
        print("❌ Yetkisiz bağlantı (Follow-up):", client_ip)
        return

    if token != SECRET_KEY:
        await websocket.close()
        print("❌ Yetkisiz token (Follow-up)")
        return
    # ------------------------------
    ...
    try:
        ...
        # RAG işlemini beklerken WebSocket'in ölmediğinden emin olalım
        cevap = await asyncio.to_thread(
            soru_sor, 
            req_data['question'], 
            format_history(req_data.get('history', []))
        )
        
        await websocket.send_json({"answer": cevap})
        # ✅ Mesajı gönderdikten sonra bir saniye bekle ki NestJS veriyi alabilsin
