...
def ai_answer(query, results):
    context_list = []
    for r in results[:5]:
        context_list.append(f"Başlık: {r.get('title','Bilinmiyor')}\nİçerik: {r.get('content','')[:500]}\nURL: {r.get('url','#')}")

    context = "\n\n".join(context_list)

    sys_prompt = f"""
Sen gelişmiş bir arama motoru asistanısın. Kullanıcıya verilen belgeler üzerinden özet çıkar ve önerileri JSON formatında ver.
Öneriler yalnızca verilen belgelerdeki başlık ve URL'lerden oluşmalı. Hiçbir sahte link üretme.
Örnek JSON:
{{
  "summary": "Buraya özet yaz",
  "suggestions": [
    {{"title": "{results[0]['title']}", "url": "{results[0]['url']}"}} 
    {", ".join([f'{{"title": "{r["title"]}", "url": "{r["url"]}"}}' for r in results[1:5]])}
  ]
}}
"""
    user_prompt = f"Soru:\n{query}\n\nDökümanlar:\n{context}"
...
def ai_answer_eski(query, results):
    context_list = []
    for r in results[:5]:
